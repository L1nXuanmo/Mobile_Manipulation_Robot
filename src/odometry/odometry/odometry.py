...
class Odometry(Node):
    ...
    def __init__(self):
        ...
        self._vw_pub = self.create_publisher(Twist, "/wheel_odom", 10)
        # Store the path here
        self._path = Path()
        ...
        self._pose_pub = self.create_publisher(PoseStamped, "/recent_odom", 10)
    ...
    def encoder_callback(self, msg: Encoders):
        ...
        dt = 50 / 1000  # update interval in seconds (50 ms)
        ticks_per_rev = 48 * 64  # number of ticks per revolution
        wheel_radius = 0.04921  # radius of the wheel in meters 
        base = 0.31  # distance between the wheels in meters 

        # Scaling factor to transform encoder ticks to wheel rotation angle
        ...
        delta_phi_left = K * delta_ticks_left
        delta_phi_right = K * delta_ticks_right

        # Calculate the distance each wheel has traveled
        D_left = wheel_radius * delta_phi_left
        D_right = wheel_radius * delta_phi_right

        # Calculate the linear and angular velocities
        v = (D_right + D_left) / (2 * dt)
        omega = (D_right - D_left) / (base * dt)
        ...
        self.get_logger().info(f'w:{omega}')
