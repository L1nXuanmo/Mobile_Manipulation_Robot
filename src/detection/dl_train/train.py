...
def compute_loss(
    ...
    Returns:
        Tuple of three separate loss terms:
            ...
            cls_ce: Cross-Entropy Loss of classifications
    """
    ...
    cls_ce = nn.functional.cross_entropy(pred_class_vector, target_class_vector) 
...
def train(device: str = "cpu") -> None:
    ...
    """
    wandb.init(project="detector_baseline+category")
    ...
    dataset = CocoDetection(
        ...
        annFile="./dd2419_23_data/dd2419_23_data_b/annotations/train.json",
        transforms=input_transforms,
    )
    dataset = wrap_dataset_for_transforms_v2(dataset)
    val_dataset = CocoDetection(
        root="./dd2419_23_data/dd2419_23_data_b/train",
        annFile="./dd2419_23_data/dd2419_23_data_b/annotations/validation.json",
    ...
    with open('dd2419_23_data/dd2419_23_data_b/annotations/training_origin.json', 'r') as file:
        data = json.load(file)
    cls_dic = data['categories']
    ...
    wandb.config.weight_cls = WEIGHT_CLS
    ...
    while current_iteration <= NUM_ITERATIONS:
        for images, anns in dataloader:
            ...
            reg_mse, pos_mse, neg_mse, cls_ce= compute_loss(out, target_batch)
            loss = WEIGHT_POS * pos_mse + WEIGHT_REG * reg_mse + WEIGHT_NEG * neg_mse + WEIGHT_CLS * cls_ce
            ...
            if current_iteration % VALIDATION_ITERATION == 1 and show_test_images:
                ...
                with torch.no_grad():
                    ...
                    bbs = detector.out_to_bbs(out, threshold = 0.8)
    ...
    model_path = "./model/Iteration{}-LearningRate{}-Time{}.pt".format(NUM_ITERATIONS, LEARNING_RATE, run_name)
...
def validate(
    ...
    with torch.no_grad():
        count = total_pos_mse = total_reg_mse = total_neg_mse = total_cls_ce = loss = 0
        image_id = ann_id = 0
        for val_images, val_anns in val_dataloader:
            ...
            reg_mse, pos_mse, neg_mse, cls_ce = compute_loss(val_out, val_target_batch)
        ...
        wandb.log(
            {
                ...
                "val loss cls": (total_cls_ce / count),
