...
class Detector(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.out_channels = 5 + NUM_CATEGORIES
        ...
        # 1280x15x20 -> 5+Nx15x20, where each element 5+N channel tuple corresponds to
    ...
    def out_to_bbs(
        ...
        Args:
            out (torch.tensor):
                The output tensor encoding the predicted bounding boxes.
                Shape (N, 5+N, self.out_cells_y, self.out_cells_y).
        ...
        for o in out:
            ...
            for bb_index in bb_indices:
                ...
                bb_category = torch.argmax(o[5:, bb_index[0], bb_index[1]])
                ...
                # print(o[5, bb_index[0], bb_index[1]].item())
                img_bbs.append(
                    {
                        "width": width,
                        "height": height,
    ...
    def anns_to_target_batch(self, anns: Tuple[dict]) -> Tuple[torch.Tensor]:
        ...
        for i, ann in enumerate(anns):
            ...
            # for j, box in enumerate(ann["boxes"]):
                ...
                one_hot_encoding = nn.functional.one_hot(torch.tensor([int(label)]), num_classes=NUM_CATEGORIES).squeeze()
                target[i, 5:(5+NUM_CATEGORIES), y_ind, x_ind] = one_hot_encoding
